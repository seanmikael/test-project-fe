<template>
  <div>
    ...
    <table v-if="usersData.length > 0" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700"
      ...
      <tbody class="divide-y divide-gray-200">
        <tr v-for="user in usersData" :key="user.id">
          <td class="py-3 pl-4">
            <div class="flex items-center h-5">
              <input id="hs-table-pagination-checkbox-1" v-model="selectedUsers" type="checkbox" :value="user.id"
                class="border-gray-200 rounded text-blue-600 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:border-gray-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800" />
          ...
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <button class="text-blue-500 mr-4" data-hs-overlay="#hs-modal-update" @click="$emit('updateUser', user)">
              Edit
            </button>
            <button class="text-red-500" data-hs-overlay="#hs-danger-alert" @click="deleteHandler(user)">
              Delete
            </button>
          </td>
        </tr>
    ...
    <user-delete-modal :selected-users="selectedUsers" :selected-user="selectedUser" />
  </div>
</template>
