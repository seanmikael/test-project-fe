    ...
    async deleteUsers() {
      try {
        if (this.selectedUsers.length === 0 || !this.selectedUsers) {
          const res = await axios.delete(`http://localhost:8000/api/user/${this.selectedUser}`)
          if (res.status === 200) {
