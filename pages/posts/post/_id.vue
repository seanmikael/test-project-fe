<template>
  <div>
    <div
      ...
      <!-- BREADCRUMBS START-->
      <ol class="flex items-center whitespace-nowrap min-w-0 mb-2" aria-label="Breadcrumb">
        <li class="text-sm">
          <nuxt-link :to="{ name: 'posts' }" class="flex items-center text-gray-500 hover:text-blue-600">
            Posts
        ...
        <div class="rounded-xl border p-5 shadow-md w-full bg-white">
              ...
              <div class="flex space-x-4 md:space-x-8">
                <div class="flex flex-col mb-5">
                  <div class="w-full h-96">
                    <img class="object-cover w-full h-full" :src="getImageUrl(image_path)" alt="Post Image" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="flex justify-end">
            <div v-if="status === 'Draft' && !edit" class="flex">
              <button class="btn border border-gray-300 p-1 px-4 font-semibold cursor-pointer text-gray-500"
                @click="editState" type="submit">
                Edit
              </button>
              <button
                class="btn border border-indigo-500 p-1 px-4 font-semibold cursor-pointer text-gray-200 ml-2 bg-indigo-500"
                type="submit" @click="publishPost">
                Publish
              </button>
            </div>
            <div v-if="status === 'Published' && !edit" class="flex justify-end align-bottom items-start">
              <button class="btn border border-gray-300 p-1 px-4 font-semibold cursor-pointer text-gray-500"
                @click="editState" type="submit">
                Edit
              </button>
            ...
            <div v-if="edit" class="flex justify-end">
              <button class="btn border border-gray-300 p-1 px-4 font-semibold cursor-pointer text-gray-500" type="submit"
                @click="cancelState">
                Cancel
              </button>
              <button
    ...
    <div v-if="edit"
      ...
      <div class="flex flex-row p-4 md:p-10">
        <div class="flex w-2/3">
          <form class="w-full" @submit.prevent="updatePost">
            <div class="editor flex flex-col text-gray-800 p-4 max-w-3xl">
              <input v-model="title" class="title bg-gray-100 border border-gray-300 p-2 mb-4 outline-none"
                spellcheck="false" placeholder="Title" type="text" />
              <textarea v-model="content" maxlength="300"
                class="description mb-4 bg-gray-100 sec p-3 h-60 border border-gray-300 outline-none" spellcheck="false"
                placeholder="Describe everything about this post here" @input="updateCount"></textarea>
              <label for="hs-select-label" class="block text-sm font-medium mb-2 dark:text-white">Category</label>
              <div class="flex flex-row">
                <select v-model="categoryEdit"
                  ...
                  <option v-for="categoryOption in categoriesData" :key="categoryOption.id" :value="categoryOption.id">
        ...
        <div class="flex w-1/3">
          <div class="flex flex-col">
            <div v-if="image_path && !previewUrl"
